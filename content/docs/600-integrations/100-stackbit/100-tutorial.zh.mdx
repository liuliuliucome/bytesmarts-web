...
<Callout>
  Note that best practice generally leads to breaking up each object in the
  `models` property into individual files, but are combined here for context.
</Callout>
...
<Callout>
  Notice that we didn't have to define the `title` field. We also didn't have to
  define properties like `name` or `filePathPattern` on the models, as they were
  already inferred by this function.
</Callout>
...
<Callout>
  We recommend using
  [`withContentlayer`](/docs/reference/next-contentlayer#withcontentlayer) when
  using Next.js to run `contentlayer dev` in sync with `next dev`.
</Callout>
